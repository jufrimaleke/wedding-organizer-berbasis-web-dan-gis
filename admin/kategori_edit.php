...
<body>
	...
	<div class="section">
		<div class="container">
			...
			<div class="box">
				<?php 
				$query = mysqli_query($con, "SELECT * FROM tb_category WHERE category_id = '".$_GET['idk']."' ");
				if (mysqli_num_rows($query) == 0) {
					echo "<script>window.location='kategori'</script>";
				}else {
				$data = mysqli_fetch_object($query);
				}
				?>
				<form method="POST" action="">
					<input type="text" name="kategori" value="<?= $data->category_name ?>" class="input-control" required>
					<input type="submit" name="submit" value="Simpan" class="btn3">
					<button class="btn2"><a href="kategori">Batal</a></button>
				</form>
				<?php 
